...
from typing import TypeVar, Optional, Iterable, List, Self
...
class TransactionManager(models.Manager):
    ...
    def get_summary_for(self, year: int, month: int, iban: Optional[str], user: AbstractBaseUser):
        this_month = datetime.date(year, month, 1)
        this_month_end = this_month + relativedelta(months=1, day=-1)
        last_month = this_month + relativedelta(months=-1)
        next_month = this_month + relativedelta(months=1)

        ibans = [t['iban'] for t in
                 self.filter(user=user).values('iban').annotate(amount_of_transactions=Count('id')).order_by(
                     '-amount_of_transactions')]
        transactions = self.filter(user=user, date__gte=last_month, date__lt=next_month).all()
        ...
        for transaction in transactions:
            ...
            is_last_month = date.year == last_month.year and date.month == last_month.month
            is_this_month = not is_last_month
            amount = transaction.amount
            week = date.isocalendar().week
            is_target_iban = iban == transaction.iban

            if is_target_iban and is_last_month and transaction.is_fixed() and transaction.is_income():
                summary.income += amount
                summary.income_transactions.append(transaction)

            if is_target_iban and is_last_month and transaction.is_fixed() and transaction.is_expense():
            ...
            if is_this_month:
                if transaction.iban not in summary.iban_balances:
                    summary.iban_balances[transaction.iban] = BalanceSummary(transaction.iban, 0)
        ...
        for week, week_summary in summary.weeks:
            budget_of_week = (summary.budget / this_month_end.day) * len(dates_per_week[week])
